...
def _build_url2(self,**qargs):
        ...
        # create signat  
        fnd=0
        bagtypes=['wallet for men','Laptop backpacks']
        #bagtypes=['Laptop backpacks']
        for bagt in bagtypes:
            baz=[]
            f=open(bagt.replace(" ","_"),'r')
            for i in f:
              baz=i.split("|")
              #b=re.match(r' *([a-zA-z ]*) *\(([0-9]*)\) *',i)
             # if b:
             #      baz.append(b.group(1)[:-1])
#            fbrnds=open(bagt+"laptopbrands",'r')
 #           for i in fbrnds:
  #              if i[:-1].strip():
   #               baz.append(i[:-1])
            for subbrnd in baz:
             ...
             if subbrnd.strip():
              try:
                 cursor.execute(" SELECT count(*) FROM information_schema.TABLES WHERE  (TABLE_NAME = '"+bagt.replace(" ","_")+subbrnd.replace(" ","")+"')")
              except UnicodeDecodeError,cursor.ProgrammingError:
                      print "error at haha"
                      continue
              dropa=cursor.fetchall()
              if int(dropa[0][0])==1:
                try:
                 cursor.execute("drop table `"+bagt.replace(" ","_")+ subbrnd.replace(" ","")+"`" )
                except UnicodeDecodeError,cursor.ProgrammingError:
                      print "error at haha"
                      continue
              print subbrnd
              if (not subbrnd):
                  continue 
              try:                
                  cursor.execute("Create table `"+bagt.replace(" ","_")+subbrnd.replace(" ","")+"`(id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,amzlink VARCHAR(300) NOT NULL,itemlink1 VARCHAR(300),itemlink2 VARCHAR(300),itemlink3 VARCHAR(300),itemlink4 varchar(300),amzimg VARCHAR(300) NOT NULL,amzimg0 VARCHAR(200),amzimg1 VARCHAR(200),amzimg2 VARCHAR(200),amzimg3 VARCHAR(200),amzimg4 VARCHAR(200),amzimg5 VARCHAR(200),price INT, brand varchar(30),color varchar(20),binding varchar(20),department varchar(20),features VARCHAR(2000),productgroup varchar(30),size varchar(30),title varchar(30),height int,width int,length int,weight int,offerprice int,salesrank int,sim0 varchar(50),sim1 varchar(50),sim2 varchar(50),sim3 varchar(50),sim4 varchar(50),asin varchar(50))")
              except UnicodeDecodeError,cursor.ProgrammingError:
                      print "error at haha"
                      continue
        ...
        # On python3, HMAC needs bytes for key and msg.
               ...
               if(kit.xpath('//atom:Error',namespaces=ns)):
                  error3.write(subbrnd+" "+bagt)
               ...
               for k in items:
                 ...
                 if k.xpath('.//atom:PackageDimensions/atom:Weight/text()', namespaces=ns):
                    ...
                    weight="0"
                 try: 
                      print "Insert into  `"+bagt.replace(" ","_")+subbrnd+"`(amzlink, amzimg,price,brand,color,height,width,length,weight,offerprice) VALUES ('"+ t2+"', '"+ t1+"', "+ prc+",'"+ brand.replace("'","")+"', '"+ str(color)+"', "+ height+", "+ width+", "+ length+", "+ weight+","+offerprice+" s"+size+" tit "+title+")"
                      cursor.execute("Insert into  `"+bagt.replace(" ","_")+subbrnd.replace(" ","")+"`(amzlink,itemlink1,itemlink2,itemlink3,itemlink4, amzimg,amzimg0,amzimg1,amzimg2,amzimg3,amzimg4,amzimg5,price,brand,color,binding,department
