...
from .models import infiitems,ptmitems,snapitems
...
import re
def index(request):
    catet1=['Messenger Bags','Laptop Bags','Rucksacks','School Bags','Cabin Luggage','ToteBbags','Travel Duffles','Travel Accessories','Briefcases','Wallet for Women','Handbags','Slingbags']
...
#'http://ecx.images-amazon.com/images/I/41Lff8oitLL._AC_UL246_SR190,246_.jpg','http://ecx.images-amazon.com/images/I/41iDncnPOyL._AC_UL246_SR190,246_.jpg','http://ecx.images-amazon.com/images/I/41vGGhKQiwL._AC_UL246_SR190,246_.jpg','http://ecx.images-amazon.com/images/I/51vnaQ2JGoL._AC_UL246_SR190,246_.jpg','http://ecx.images-amazon.com/images/I/41nnRhGmCYL._AC_UL246_SR190,246_.jpg','http://ecx.images-amazon.com/images/I/417PsjAihZL._AC_UL260_SR200,260_.jpg','http://ecx.images-amazon.com/images/I/41ReiAOMZ1L._AC_UL260_SR200,260_.jpg']
    ...
    cateimgs1=[]
    cateimgs2=[]
    cate=['messenger bags','Laptop bags','rucksacks','school bags','cabin luggage','tote bags','travel duffles','travel accessories','briefcases','wallet for women','handbags','slingbags']
...
def tprice(request,category,p1,p2):
  ...
  #  cats=amznitems.objects.filter(bagt__icontains=category).all()
   ...
   # for i in cats:
    ...
    for k in brndcats:
        #    k=brndcats[0]
            if k.amzimg not in imglst and k.blk==0 and k.catval!=0:
                 brnd.append(k.brand)
                 imglst.append(k.amzimg)
                 k.amzimg=k.amzimg.replace('SL160','SL260')
                 queryitems.append(k)
...
def sbtour(request, category):
   ...
   #     if(not i.tablename): continue
    ...
    cate={'messenger bags':3,'Laptop bags':0,'rucksacks':5,'school bags':6,'cabin luggage':1,'tote bags':9,'travel duffles':2,'travel accessories':11,'briefcases':4,'wallet for women':10,'handbags':7,'slingbags':8}
   # cate=[3,0,5,6,1,9,2,11,4,10,7,8]
...
def tours(request, category):
   ...
   # for i in cats:
   #     if(not i.tablename): continue
    cate={'messenger bags':3,'Laptop backpacks':0,'rucksacks':5,'school bags':6,'cabin luggage':1,'tote bags':9,'travel duffles':2,'travel accessories':11,'briefcases':4,'wallet for women':10,'handbags':7,'slingbags':8}
   # cate=[3,0,5,6,1,9,2,11,4,10,7,8]
    ...
    for i in cats:
        ...
        for k in brndcats:
          if cate[category]==11:
            ...
            if k.amzimg not in imglst and k.catval!=0 and k.catval!=2:
                 imglst.append(k.amzimg)
                 k.title=k.title[:20]+".."
                 k.amzimg=k.amzimg.replace('SL160','SL260')
                 queryitems.append(k)
